...
titulo = customtkinter.CTkLabel(janela, text="Conversor de Moedas", font=("Comic Sans MS", 28), text_color="#7D80DA")
...
botao_converter = customtkinter.CTkButton(janela, text="Converter", command=converter_moeda, font=("", 16))

lista_moedas = customtkinter.CTkScrollableFrame(janela)

texto_cotacao_moeda = customtkinter.CTkLabel(janela, text=(""), font=("", 12))

moedas_disponiveis = nomes_moedas()

for codigo_moeda in moedas_disponiveis:
    nome_moeda = moedas_disponiveis[codigo_moeda]
    texto_moeda = customtkinter.CTkLabel(lista_moedas, text=f"{codigo_moeda}: {nome_moeda}", font=("", 12))
    texto_moeda.pack()
